...
next_antialias(const Event&, State *&)
{
   ...
   if (VIEW::peek()->get_antialias_enable())
      WORLD::message(string("Antialiasing: ENALBED Jitters: ") + tmp);
   else
      WORLD::message(string("Antialiasing: DISABLED Jitters: ") + tmp);
...
save_config(const Event &e, State *&)
{
   ...
   if (ret)
      WORLD::message("Wrote config to " + Config::JOT_ROOT() + "jot.cfg");
   else
      WORLD::message("FAILED!! Writing config to " + Config::JOT_ROOT() + "jot.cfg");
