...
map<string,TEXTUREptr>* ToonTexture_1D::_toon_texture_map = nullptr;
map<string,string>*     ToonTexture_1D::_toon_texture_remap = nullptr;

/*****************************************************************
 * Texture Remapping
 *****************************************************************/

const  char *toon_remap_base_1D = "nprdata/toon_textures/";
const char *toon_remap_fnames_1D[][2] =
{
...
/////////////////////////////////////
TAGlist *       ToonTexture_1D::_ntt_tags = nullptr;
...
ToonTexture_1D::update_tex(void)
{
   ...
   if (!_toon_texture_map) {
      ...
      int i = 0;
      while (toon_remap_fnames_1D[i][0] != nullptr) {
   ...
   if (tf == "") {
      ...
      if ((ind = _toon_texture_map->find(tf)) != _toon_texture_map->end()) {
         ...
         if (ind->second != nullptr) {
            ...
            err_mesg(ERR_LEV_INFO, "ToonTexture_1D::set_texture() - **ERROR** Previous caching failure: '%s'...", tf.c_str());
            _tex = nullptr;
            _tex_name = "";
         }
      ...
      //Haven't seen this name before...
         ...
         //Otherwise insert a failed nullptr
            ...
            err_mesg(ERR_LEV_ERROR, "ToonTexture_1D::set_texture() - *****ERROR***** Failed loading to cache: '%s'...", (Config::JOT_ROOT()+tf).c_str());
...
ToonTexture_1D::put_layer_name(TAGformat &d) const
{
   ...
   if (get_layer_name() == "")
   ...
   else
      {
         *d << get_layer_name();
         *d << " ";
         err_mesg(ERR_LEV_SPAM, "ToonTexture_1D::put_layer_name() - Wrote string: '%s'", get_tex_name().c_str());
      }
   d.end_id();
}
...
ToonTexture_1D::get_layer_name(TAGformat &d)
{
   ...
   *d >> str;      
   ...
   if (str == "NULL_STR") 
      ...
      {
         lay = str;
         err_mesg(ERR_LEV_SPAM, "ToonTexture_1D::get_layer_name() - Loaded string: '%s'", lay.c_str());
      }
   set_layer_name(lay);
...
ToonTexture_1D::put_tex_name(TAGformat &d) const
{
   ...
   if (_tex_name == "")
   ...
   else
      {
         *d << get_tex_name();
         *d << " ";
         err_mesg(ERR_LEV_SPAM, "ToonTexture_1D::put_tex_name() - Wrote string: '%s'", get_tex_name().c_str());
      }
   d.end_id();
}
...
ToonTexture_1D::get_tex_name(TAGformat &d)
{
   ...
   *d >> str;      
   ...
   if (str == "NULL_STR") 
      ...
      {
         err_mesg(ERR_LEV_SPAM, "ToonTexture_1D::get_tex_name() - Loaded string: '%s'", str.c_str());
      }
   set_tex_name(str);
