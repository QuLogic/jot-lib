...
{
  int workMask;
  int window_num = window->num;

  GLUTwindow *leftovers = NULL;

  if (window->prevWorkWin) 
  {
      leftovers = processWindowWorkList(window->prevWorkWin);

      if (!__glutWindowList[window_num] || (__glutWindowList[window_num] != window))
      {
         __glutWarning("Window destroyed before it was handled in pending queue.");
         return leftovers;
      }
  } 
  ...
  if (workMask & (GLUT_REDISPLAY_WORK | GLUT_OVERLAY_REDISPLAY_WORK | GLUT_REPAIR_WORK | GLUT_OVERLAY_REPAIR_WORK)) {
   ...
   if (!__glutWindowList[window_num] || (__glutWindowList[window_num] != window))
   {
      //__glutWarning("Window destroyed while it was being handled.");
      return leftovers;
   }
   else
   {
      ...
      if (window->workMask) 
      {
         window->prevWorkWin = leftovers;
         if (!beforeEnd) beforeEnd = &window->prevWorkWin;
         /* Leave on work list. */
         return window;
      } 
      else 
  ...
  for (;;) {
    if (__glutWindowWorkList) {
      ...
      if (work) {
        ...
        if (remainder) 
        {
           /* XXX - If a window has handled, stil had leftover events,
                    and was later destroyed, the list of remaining events
                    will be defunct... Could check linked window list against 
                    the __glutWindowList, and regerate the remainder list
                    if there's a problem. Regeneration could look at
                    windows that have a workmask, but aren't in the
