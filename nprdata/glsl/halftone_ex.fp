...
//controls the halftone screen scale and offset
...
//val,lod = [0:1]
float
get_lod_height(in float val, in float lod)
{
  float p = lod / 2.0;
  float h;
...
float
halftone(in vec2 uv, in float tone, in float spacing, in float lod)
{
  const float e = 0.15;
  // Get a floating point value between 0.0 and spacing
  float scaled_position = mod(uv.y, spacing);
  float h =
    halftone_scale * (get_lod_height(scaled_position / spacing, lod) +
                      halftone_offset);
...
mat3
matrix_derivative(in mat4 T, in vec4 point)
{
...
main()
{
  ...
  gl_FragColor = vec4(mix(stroke_color, gl_Color.rgb, t), gl_Color.a);
}
