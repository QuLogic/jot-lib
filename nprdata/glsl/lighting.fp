...
vec4
light_contribution(in gl_LightSourceParameters light,   // light parameters
  ...
  )
{
  ...
  vec3 l = normalize((light.position.w == 0.0 ? light.position :        // directional
  ...
  if (light.spotCutoff < 180.0) {       // if spotlight in effect
    ...
    t *= (sl < light.spotCosCutoff) ? 0.0 :     // out of cone
