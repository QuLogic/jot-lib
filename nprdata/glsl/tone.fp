...
bool
intersect_sphere(in vec3 o,     // ray origin
...
shadow_effect(in occluder_t occ, in vec4 l, in bool is_directional)
{
  ...
  if (occ.is_active) {
    ...
    if (intersect_sphere((occ.xf * P).xyz, (occ.xf * l).xyz, t0, t1)) {
      if (is_directional) {
        // directional
        ret = ((t1 < 0.0) ? 1.0 : (t0 < 0.0) ? 0.0 : t0 / t1);
      } else {
        // positional
        ret = ((t1 < 0.0 || t0 > 1.0 || (t0 < 0.0 && t1 > 1.0)) ? 1.0 :
               (t1 > 1.0) ? t0 : (t0 < 0.0) ? 0.0 : t0 / t1);
      }
...
vec4
light_contribution(in gl_LightSourceParameters light,   // parameters for current light
...
float
sil_val(vec3 n, vec3 v, layer_t layer)
{
  // return 1 at silhouette, falling to 0 away from silhouette
  return smoothstep(layer.s0, layer.s1, 1.0 - dot(n, v));
}

float
layer_val(in gl_LightSourceParameters light,    // parameters for current light
