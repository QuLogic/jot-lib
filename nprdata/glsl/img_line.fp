// image line shader fragment program.
...
void
compute_quadric_coefficients(in vec2 uv, in int channel, in float half_width,
                             out float max_curv, out float axis_dist,
                             out vec2 axis)
{
  ...
  // H * A
  T[0] = (H0 * A[0]) + (H1 * A[1]) + (H2 * A[2]) + (H3 * A[3]) + (H4 * A[4])
       + (H5 * A[5]) + (H6 * A[6]) + (H7 * A[7]) + (H8 * A[8]);
...
float
compute_alpha(in float center_val, in float max_curv, in float dist,
              in int sil_cnt, in float c_thd0, in float c_thd1)
{
  float val;
...
draw_line_pixel(in vec2 pix)
{
  ...
  } else if (line_mode == 1 || line_mode == 4 || line_mode == 6
             || line_mode == 7) {
    pix2 = pix;
