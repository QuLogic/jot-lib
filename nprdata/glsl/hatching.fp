...
float
get_tone(float min_tone, int highlight, int mychannel)
{
  vec2 uv =
    vec2(gl_FragCoord.x / float (width), gl_FragCoord.y / float (height));
  float tone =
    (mychannel == 0) ? texture2D(tone_map, uv).r
                     : (mychannel == 1) ? texture2D(tone_map, uv).g
                                        : (mychannel == 2) ? texture2D(tone_map, uv).b
                                                           : texture2D(tone_map, uv).a;
  return (highlight == 1) ? clamp(1.0 - (tone + min_tone), 0.0, 1.0) : (tone + min_tone);
}

float
interp(float x0, float y0, float x1, float y1, float x)
{
...
float
dropoff(float x0, float y0, float x1, float y1, float x2, float x)
{
  // required: x0 < x1 < x2
  //
...
vec2
get_noise2(vec2 u, float amp, float freq)
{
...
main()
{
  ...
  for (int i = 0; i < 4; ++i) {
    if (visible[i] == 1) {
      new_col =
        mix(new_col, color_ub(color[i].x, color[i].y, color[i].z),
            get_layer(i));
    }
  }
