...
void
compute_ridge_properties(in vec2 uv, out float max_curv, out float axis_dist,
                         out vec2 axis)
{
  ...
  //
  a = (H0 * T[0]) + (H1 * T[1]) + (H2 * T[2]) + (H3 * T[3]) + (H4 * T[4])
    + (H5 * T[5]) + (H6 * T[6]) + (H7 * T[7]) + (H8 * T[8]);
...
float
compute_line_opacity(in float center_val, in float max_curv,
...
//
vec4
draw_line_pixel(in vec2 center_pix)
{
  vec2 scale = vec2(x_1, y_1), diff;
