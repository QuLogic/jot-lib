...
vec4
light_contribution(gl_LightSourceParameters gl_params,  // parameters for current light
...
vec4
basecoat_color(gl_LightSourceParameters gl_params,      // parameters for current light
               vec3 n,          // unit surface normal, eye space
               vec3 v           // unit view vector, eye space
  )
{
  ...
  if (is_enabled) {
    ...
    col =
      vec4(base_color.r * ret, base_color.g * ret, base_color.b * ret, 1.0);
