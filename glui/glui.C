...
int GLUI::reposition( int x, int y )
{
  int old_glut_window;
...
GLUI_Main::create_standalone_window( char *name, int x, int y )
{
  ...
  // XXX - is this right?
  glutInitDisplayMode( GLUT_RGB | GLUT_SINGLE | GLUT_STENCIL ); /* | GLUT_DOUBLE );  */
...
void GLUI_Master_Object::block_gluis_by_gfx_window_id( int window_id )
{
   ...
   while( node ) 
   {
      if (window_id == -1)
      {
         ((GLUI*)node)->set_blocked(1);
         ((GLUI*)node)->refresh();
      }
      else if ( ((GLUI*)node)->get_main_gfx_window() == window_id )
      {
         ((GLUI*)node)->set_blocked(1);
         ((GLUI*)node)->refresh();
      }
...
void    GLUI_Main::mouse(int button, int state, int x, int y)
{
  ...
  if ( button == GLUT_LEFT_BUTTON ) 
  {
      ...
      if ( mouse_button_down AND active_control != NULL AND state == GLUT_UP ) 
      {
         ...
         //CCC glutSetCursor( GLUT_CURSOR_LEFT_ARROW );
         glutSetCursor( GLUT_CURSOR_INHERIT );
         if ( active_control AND  active_control->active_type == GLUI_CONTROL_ACTIVE_MOUSEDOWN 
             AND 0)
         {
	         /*** This is a control that needs to be disactivated when the mouse button is released ****/
            disactivate_current_control();
         }
      }
      else 
...
void    GLUI_Main::motion(int x, int y)
{
   ...
   else if ( middle_button_down AND middle_button_control != NULL ) 
   {
      callthrough = middle_button_control->general_mouse_held_down_handler(GLUI_MOUSE_MIDDLE,x,y,control==middle_button_control);
   }
   else if ( right_button_down AND right_button_control != NULL ) 
   {
      callthrough = right_button_control->general_mouse_held_down_handler(GLUI_MOUSE_RIGHT,x,y,control==right_button_control);
   }
